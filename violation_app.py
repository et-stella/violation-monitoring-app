...
def detect_condition_2(df):
    ...
    for sap, group in df.groupby('SAPID'):
        ...
        for date in group['PurchaseDate']:
            ...
            qty_by_article = window.groupby(['SAPID', 'Article'])['NetQuantity'].sum().reset_index()
            if qty_by_article[qty_by_article['NetQuantity'] != 0]['Article'].nunique() > 5:
                for _, row in qty_by_article.iterrows():
                    article = row['Article']
                    qty = row['NetQuantity']
